		...
		.extend(
				$.fn,
				{
					...
					function createJerichoTab() {
						 ...
						 */
						var jerichotab = $(
								'<div class="jericho_tab"><div class="tab_pages" ><div class="tabs"><ul /></div></div><div class="tab_content"><div id="jerichotab_contentholder" class="content" style="overflow-y:hidden;height:100%;"/></div></div>')
								.appendTo($(opts.renderTo));
						//apply contentcss to the contentholder
						$('.tab_content>.content', jerichotab).css(
								opts.contentCss);
						$('.tab_content>.content', jerichotab).css(
								'height','100%');
						...
						$.fn.jerichoTab = {
							...
							tagGuid = (typeof tagGuid == 'undefined' ? 0
									: tagGuid + 1);
							...
							if (ps.tabFirer != null) {
								var jerichotabindex = ps.tabFirer
										.attr('jerichotabindex');
								if (typeof jerichotabindex != 'undefined'
										&& $('#jerichotab_' + jerichotabindex).length > 0)
									return $.fn.setTabActive(jerichotabindex)
											.adaptSlider().loadData();
							 ...
							 */
							var newTab = $(
									'<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_'
											+ curIndex
											+ '" dataType="'
											+ ps.data.dataType
											+ '" dataLink="'
											+ ps.data.dataLink
											+ '">'
											+ '<div class="tab_left"  style="width:'
											+ (opts.tabWidth - 5)
											+ 'px;"  >'
											+ '<div class="tab_icon" style="'
											+ (ps.iconImg == '' ? 'backrground:none'
													: 'background-image:url(' + ps.iconImg + ')')
											+ '">&nbsp;</div>'
											+ '<div class="tab_text" title="'
											+ ps.title
											+ '"  style="width:'
											+ (opts.tabWidth - 50)
											+ 'px"  >'
											+ ps.title
													.cut(opts.tabWidth / 10 - 1)
											+ '</div>  '
											+ '<div class="tab_close">'
											+ (ps.closeable ? '<a href="javascript:void(0)" title="关闭">&nbsp;</a>'
													: '')
											+ '</div>'
											+ '</div>'
											+ '<div class="tab_right">&nbsp;</div>'
											+ '</li>').appendTo(
									$.fn.jerichoTab.tabpage)
									.css('opacity', '0').applyHover()
									.applyCloseEvent().animate( {
										'opacity' : '1',
										width : opts.tabWidth
									}, function() {
							...
							tabHash = (typeof tabHash == 'undefined' ? []
									: tabHash);
							tabHash.push( {
								index : curIndex,
					...
					$(window)
							.resize(
									function() {
										$.fn.jerichoTab.tabPageWidth = $(
												'.tab_pages',
												$.fn.jerichoTab.master).width()
												- (($('.jericho_slider').length > 0) ? 38
														: 0);
										$('.tabs', $.fn.jerichoTab.master)
												.width(
														'100%')//$.fn.jerichoTab.tabPageWidth)
												.applySlider().adaptSlider();
									})
				...
				//activate the tag(orderkey is the tab order, start at 1)
					setTabActiveByOrder : function(orderKey) {
						var lastTab = $.fn.jerichoTab.tabpage.children('li')
								.filter('.tab_selected');
						if (lastTab.length > 0)
							lastTab.swapTabEnable();
						return $('#jericho_tabs').filter(
								':nth-child(' + orderKey + ')').swapTabEnable();
					},
						...
						return this
								.each(function() {
									...
									document.getElementById(jerichotabiframe).onload = function() {
										...
										!!onComleted ? onComleted(arguments[1])
												: true;
										$.fn.removeLoader();
									}
								});
					...
					loadData : function() {
						return this.each(function() {
							//show ajaxloader first
								...
								$('#jerichotab_contentholder').children(
										'div[class=curholder]').attr('class',
										'holder').css( {
									'display' : 'none'
								});
								var holder = $(holderId);
								if (holder.length == 0) {
									...
									holder = $(
											'<div  style="height:100%;"  class="curholder" id="' + holderId
													.replace('#', '') + '" />')
											.appendTo(
													$('#jerichotab_contentholder'));
								...
								function load(c) {
									...
									case 'formtag':
										...
										var clone = $(dataLink).clone(true)
												.appendTo(c).css('display',
														'block');
										removeLoading(holder);
										break;
									case 'html':
										//load HTML from page
										holder.load(dataLink + '?t='
									...
									case 'ajax':
										...
										$
												.ajax( {
													...
													error : function(r) {
														holder
																.html('error! can\'t load data by ajax');
														removeLoading(holder);
													},
													success : function(r) {
														holder.html(r);
									...
									$(me)
											...
											.mouseup(function() {
												//filter the sliders in order to prevent users from sliding
													...
													var offLeft = parseInt($.fn.jerichoTab.tabpage
															.css('left'));
													//the max css(left) of tabpage
													var maxLeft = tabHash.length
															* $.fn.jerichoTab.tabWidth
															- $.fn.jerichoTab.tabPageWidth
															+ 38;
													switch ($(me).attr('pos')) {
													case 'left':
																			...
																			function() {
																				$(
																						...
																						.attr(
																								{
																									'slide' : 'no',
																									'class' : 'jericho_sliders jericho_sliderleft_disable'
																								});
																			});
														$(
																'.jericho_sliders[pos=right]')
																.attr(
																		{
																			'slide' : 'yes',
													...
													case 'right':
														...
														if (offLeft
																- $.fn.jerichoTab.tabWidth > -maxLeft)
															$.fn.jerichoTab.tabpage
																	.animate( {
																		left : offLeft
																				- $.fn.jerichoTab.tabWidth
																			...
																			function() {
																				$(
																						...
																						.attr(
																								{
																									'slide' : 'no',
																									'class' : 'jericho_sliders jericho_sliderright_disable'
																								});
																			});
														$(
																'.jericho_sliders[pos=left]')
																.attr(
																		{
																			'slide' : 'yes',
					...
					applySlider : function() {
						return this.each(function() {
							if (typeof tabHash == 'undefined'
									|| tabHash.length == 0)
								return;
							//get the offwidth of tabpage
								var offWidth = tabHash.length
										* $.fn.jerichoTab.tabWidth
										- $.fn.jerichoTab.tabPageWidth + 38;
								if (tabHash.length > 0 && offWidth > 0) {
									...
									$.fn.jerichoTab.tabpage
											...
											.css(
													{
														width : '100%'//$.fn.jerichoTab.tabPageWidth - 38
													});
													...
													function() {
														...
														if ($('.jericho_sliders').length <= 0) {
															$.fn.jerichoTab.tabpage
																	.parent()
																	.before(
																			$('<div class="jericho_sliders jericho_sliderleft_enable" slide="yes" pos="left" />'));
															$.fn.jerichoTab.tabpage
																	.parent()
																	.after(
																			$('<div class="jericho_sliders jericho_sliderright_disable" pos="right" slide="yes" />'));
															$(
																	'.jericho_sliders')
																	.attachSliderEvent();
														}
									...
									$.fn.jerichoTab.tabpage.parent().css( {
										width : '100%'//$.fn.jerichoTab.tabPageWidth
									});
									$.fn.jerichoTab.tabpage
											.css(
													{
						...
						return this
								.each(function() {
									if ($('.jericho_sliders').length > 0) {
										...
										var space_l = $.fn.jerichoTab.tabWidth
												* index + offLeft;
										//calculate the distance from the right side of tabpage
										var space_r = $.fn.jerichoTab.tabWidth
												* (index + 1) + offLeft;
										//window.console && console.log(space_l + '||' + space_r);
										function setSlider(pos, enable) {
											$(
													'.jericho_sliders[pos=' + pos + ']')
													.attr(
															{
																'slide' : (enable ? 'yes'
																		: 'no'),
																'class' : 'jericho_sliders jericho_slider'
																		+ pos
																		+ '_'
																		+ (enable ? 'enable'
																				: 'disable')
										...
										if ((space_l < 0 && space_l > -$.fn.jerichoTab.tabWidth)
												&& (space_r > 0 && space_r < $.fn.jerichoTab.tabWidth)) {
											//left
											$.fn.jerichoTab.tabpage
													.animate(
															{
																left : -$.fn.jerichoTab.tabWidth
																		* index
															},
															function() {
																if (index == 0)
										...
										if ((space_l < tabWidth && space_l > tabWidth
												- $.fn.jerichoTab.tabWidth)
												&& (space_r > tabWidth && space_r < tabWidth
														+ $.fn.jerichoTab.tabWidth)) {
											//right
											$.fn.jerichoTab.tabpage
													.animate(
													...
													$(me)
															.animate(
																	...
																	function() {
																		...
																		var lastTab = $.fn.jerichoTab.tabpage
																				.children(
																						'li')
																				.filter(
																						'.tab_selected');
					...
					applyHover : function() {
						return this.each(function() {
							$(this).hover(function() {
								if ($(this).hasClass('tab_unselect'))
									$(this).addClass('tab_unselect_h');
							}, function() {
								if ($(this).hasClass('tab_unselect_h'))
									$(this).removeClass('tab_unselect_h');
							}).mouseup(function() {
								...
								//select this tab and hide the last selected tab
									var lastTab = $.fn.jerichoTab.tabpage
											.children('li').filter(
													'.tab_selected');
									lastTab.attr('class',
											'jericho_tabs tab_unselect');
									$(
											'#jerichotabholder_' + lastTab
													.attr('id').replace(
															'jerichotab_', ''))
											.css( {
												'display' : 'none'
					...
					swapTabEnable : function() {
						return this.each(function() {
							if ($(this).hasClass('tab_selected')) {
								$(this).swapClass('tab_selected',
										'tab_unselect');
							} else if ($(this).hasClass('tab_unselect')) {
								$(this).swapClass('tab_unselect',
										'tab_selected');
							}
						});
					},
						...
						return this.each(function() {
							...
							case 'righttag':
								...
								if ($('#jerichotab_contentholder>.righttag').length == 0)
									$('<div class="righttag">Loading...</div>')
											.css(
													{
														left : $.fn.jerichoTab.tabPageWidth - 130
													}).appendTo($(this));
								else {
									//$('#jerichotab_contentholder>.righttag').css({ display: 'block' });
									$('#jerichotab_contentholder>.righttag')
											.css( {
												display : 'none'
											});
								}
					...
					removeLoader : function() {
						switch ($.fn.jerichoTab.loader) {
						case 'usebg':
							$('#jerichotab_contentholder').removeClass(
									'loading');
							break;
						case 'righttag':
