     ...
     $("#filterdept").click(function(){
			 ...
			 if(a!=null){ var valueStr="";
              ...
              if   (a[i].checked) {
                ...
                if(confirm("真的要回退吗？")){
		             var url="<@spring.url "/tasklist/rollbackdept.nsf" />?userId=${user.id}&deptids="+valueStr;
				       $.post(url,function(data){
				           window.location.href='<@spring.url "/tasklist/deptInitList.nsf" />?&userId=${user.id}';
				        });
                  }
                   }else{
                   alert("至少选择一个！");
     ...
     $("#authordept").click(function(){
			 ...
			 if(a!=null){ var valueStr="";
              ...
              if   (a[i].checked) {
                ...
                if(confirm("真的要通过验证吗？")){
                      var url="";
			           window.location.href='<@spring.url "/tasklist/passdept.nsf" />?userId=${user.id}&deptids='+valueStr;
			            $.post(url,function(data){
				           window.location.href='<@spring.url "/tasklist/deptInitList.nsf" />?userId=${user.id}';
				        });
                }}else{
    ...
    $(function(){
     ...
     $("#pagesizego").click(function(){
         var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/tasklist/deptInitList.nsf" />?userId=${user.id}&pagesize='+pagesizegovalue;
  	});
  });
    $(function(){
     $("#pagego").click(function(){
           ...
           var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/tasklist/deptInitList.nsf" />?userId=${user.id}&page='+pagegovalue+'&pagesize='+pagesizegovalue;
  	});
  });
			...
			<td colspan="8">
				<table width="100%"  id="table1" border=1px; cellspacing="0" cellpadding="0" bordercolordark="#FFFFFF" >
										...
										<#list deptlist as dept>
											...
											<td >${dept.deptCode}</td>
											<td >${dept.name}</td>
		                                    <td ><#if  dept.parent ?? >${dept.parent.name}</#if></td>
