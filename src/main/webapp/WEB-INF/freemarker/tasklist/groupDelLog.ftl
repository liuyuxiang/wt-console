...
//<![CDATA[
 ...
 $(function(){
     $("#pagesizego").click(function(){
             userlog.action="<@spring.url "/tasklist/groupDelLog.nsf" />";
             $("#userlog").submit();
					...
					<tr>
						...
						<td colspan="3">
							<div align="right">页次${logpage.getCurrentPage()}/${logpage.lastPage}
								<a id="home" href="<@spring.url "/tasklist/groupDelLog.nsf"/>?page=1&pagesize=${logpage.getPageSize()}">首页</a>
								<#if logpage.isHasPreviousPage()>
								<a id="previous" href="<@spring.url "/tasklist/groupDelLog.nsf"/>?page=${logpage.getPreviousPage()}&pagesize=${logpage.getPageSize()}">上页</a><#else>上页</#if>
								<#if logpage.isHasNextPage()>
								<a id="next" href="<@spring.url "/tasklist/groupDelLog.nsf"/>?page=${logpage.getNextPage()}&pagesize=${logpage.getPageSize()}">下页</a><#else>下页</#if>
								<a id="last" href="<@spring.url "/tasklist/groupDelLog.nsf"/>?page=${logpage.getLastPage()}&pagesize=${logpage.getPageSize()}">尾页</a>
							</div>
						</td>
					</tr>
</table>
