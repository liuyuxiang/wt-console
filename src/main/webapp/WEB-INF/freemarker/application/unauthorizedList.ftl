  ...
  $(function(){
     $("#pagesizego").click(function(){
         var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/application/unauthorizedList.nsf" />?id=${application.uuid}&pagesize='+pagesizegovalue;
  	});
  });
    $(function(){
     $("#pagego").click(function(){
           ...
           var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/application/unauthorizedList.nsf" />?id=${application.uuid}&page='+pagegovalue+'&pagesize='+pagesizegovalue;
  	});
  });
  $(function(){
					 ...
					 <tr>
									<td colspan="8" width="100%" align="left" valign="top">
											<table width="100%" style="background-color:#DAE9F9;font-size:12px;">
												<tr>
													<td height="18" width="30%">&nbsp;【${application.name}】未授权列表</td>
													...
													<td height="21" align="right" width="18%">
														<input id="movegroupuser" type="button" class="button"  value="授 权" >
													</td>
												</tr>
											</table>
									</td>
											...
											<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:12px;">
												<tr>
													...
													<div align="right">页次${javapage.getCurrentPage()}/${javapage.lastPage}
												      <a href="<@spring.url "/application/unauthorizedList.nsf"/>?id=${application.uuid}&page=1&pagesize=${javapage.getPageSize()}">首页</a>
													  <#if javapage.isHasPreviousPage()>
													  <a href="<@spring.url "/application/unauthorizedList.nsf"/>?id=${application.uuid}&page=${javapage.getPreviousPage()}&pagesize=${javapage.getPageSize()}">上页</a><#else>上页</#if>
													  <#if javapage.isHasNextPage()>
													  <a href="<@spring.url "/application/unauthorizedList.nsf"/>?id=${application.uuid}&page=${javapage.getNextPage()}&pagesize=${javapage.getPageSize()}">下页</a><#else>下页</#if>
													  <a href="<@spring.url "/application/unauthorizedList.nsf"/>?id=${application.uuid}&page=${javapage.getLastPage()}&pagesize=${javapage.getPageSize()}">尾页</a>
													 </div>
													</td>
												</tr>
											</table>
