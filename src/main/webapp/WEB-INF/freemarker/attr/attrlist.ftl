   ...
   $(function(){
     $("#pagesizego").click(function(){
         var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/attr/attrlist.nsf" />?id=${id}&type=${type}&pagesize='+pagesizegovalue;
  	});
     $("#pagego").click(function(){
           var pagegovalue=$("#pagegovalue").val();
           var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/attr/attrlist.nsf" />?id=${id}&type=${type}&page='+pagegovalue+'&pagesize='+pagesizegovalue;
  	});
     $("#addattrbutton").click(function(){
  		 window.location.href='<@spring.url "/attr/attradd.nsf"/>?id=${id}&type=${type}';
  	});
     $("#searchSubmit").click(function(){
             ...
             if(searchcontent.length>0){
               $("#f1").attr("action",'<@spring.url "/attr/search.nsf" />?id=${id}&type=${type}');
               $("#f1").submit();
             }else{
               alert("请输入查询内容!");
             }
			...
			<td width="100%" align="left" valign="top">
				<table width="100%" style="background-color:#DAE9F9;font-size:12px;">
					<tr>
						...
						<td width="10%" align="lift"><input type="button" class="button" value="增加属性" id="addattrbutton" ></td>
						<td height="21" width="52%" align="right">
							<input type="text" name="searchcontent" id="searchcontent" onkeypress="checkEvent();" value="<#if searchcontent??>${searchcontent}</#if>" size="10"> 
								...
								<#if searchType??>
									<option value="attrname" <#if searchType=="username" >selected</#if>>按属姓名称搜索</option>
			...
			<td colspan="8">
				<table width="100%"  id="table1" border=1px; cellspacing="0" cellpadding="0" bordercolordark="#FFFFFF" >
					<tr align="center" style="background-image:url(<@spring.url "/style/default/images/title_bg.gif" />);">
						<td width="40" height="21px" align="center" class="">序号</td>
					...
					<tr>
						...
						<td width="41%">
							<div align="right">页次${javapage.getCurrentPage()}/${javapage.lastPage}
								<a href="<@spring.url "/attr/attrlist.nsf"/>?id=${id}&type=${type}&page=1&pagesize=${javapage.getPageSize()}">首页</a>
								<#if javapage.isHasPreviousPage()>
								<a href="<@spring.url "/attr/attrlist.nsf"/>?id=${id}&type=${type}&page=${javapage.getPreviousPage()}&pagesize=${javapage.getPageSize()}">上页</a><#else>上页</#if>
								<#if javapage.isHasNextPage()>
								<a href="<@spring.url "/attr/attrlist.nsf"/>?id=${id}&type=${type}&page=${javapage.getNextPage()}&pagesize=${javapage.getPageSize()}">下页</a><#else>下页</#if>
								<a href="<@spring.url "/attr/attrlist.nsf"/>?id=${id}&type=${type}&page=${javapage.getLastPage()}&pagesize=${javapage.getPageSize()}">尾页</a>
							</div>
						</td>
					</tr>
				</table>
