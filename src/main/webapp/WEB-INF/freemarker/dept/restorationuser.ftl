  ...
  $(function(){
     $("#pagesizego").click(function(){
         var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/dept/restorationuser.nsf" />?id=${deptChildren.uuid}&pagesize='+pagesizegovalue;
  	});
  });
    $(function(){
     $("#pagego").click(function(){
           ...
           var pagesizegovalue=$("#pagesizegovalue").val();
  		 window.location.href='<@spring.url "/dept/restorationuser.nsf" />?id=${deptChildren.uuid}&page='+pagegovalue+'&pagesize='+pagesizegovalue;
  	});
  });
   $(function(){
			 ...
			 if(a!=null){ var valueStr="";
              ...
              if   (a[i].checked) {
                ...
                if(confirm("真的要恢复吗？")){
                        var pagegovalue=$("#pagegovalue").val();
                        var pagesizegovalue=$("#pagesizegovalue").val();
		                var url="<@spring.url "/retorationgroupuser.nsf" />?id=${deptChildren.uuid}&userids="+valueStr+"&page="+pagegovalue+"&pagesize="+pagesizegovalue;
					 ...
					 <tr>
									<td colspan="8" width="100%" align="left" valign="top">
											<table width="100%" style="background-color:#DAE9F9;font-size:12px;">
												<tr>
													<td height="18" width="30%">&nbsp;【${deptChildren.name}】被删除成员列表</td>
													<td><input  type="button" id="viewgroupuser" class="button" value="返回成员列表" > </td>
													...
													<td height="21" align="right" width="18%">
														<input id="retorationuser" type="button" class="button"  value="恢 复" >
													</td>
												</tr>
											</table>
									</td>
											...
											<table width="100%" border="0" cellspacing="0" cellpadding="0" style="font-size:12px;">
												<tr>
													...
													<div align="right">页次${javapage.getCurrentPage()}/${javapage.lastPage}
												      <a href="<@spring.url "/dept/restorationuser.nsf"/>?id=${deptChildren.uuid}&page=1&pagesize=${javapage.getPageSize()}">首页</a>
													  <#if javapage.isHasPreviousPage()>
													  <a href="<@spring.url "/dept/restorationuser.nsf"/>?id=${deptChildren.uuid}&page=${javapage.getPreviousPage()}&pagesize=${javapage.getPageSize()}">上页</a><#else>上页</#if>
													  <#if javapage.isHasNextPage()>
													  <a href="<@spring.url "/dept/restorationuser.nsf"/>?id=${deptChildren.uuid}&page=${javapage.getNextPage()}&pagesize=${javapage.getPageSize()}">下页</a><#else>下页</#if>
													  <a href="<@spring.url "/dept/restorationuser.nsf"/>?id=${deptChildren.uuid}&page=${javapage.getLastPage()}&pagesize=${javapage.getPageSize()}">尾页</a>
													 </div>
													</td>
												</tr>
											</table>
