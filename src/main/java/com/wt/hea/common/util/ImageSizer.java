...
public class ImageSizer
{
	 ...
	 */
	public static BufferedImage resizeImage(String imgName, int type, boolean isZoom, int maxWidth,
			int maxHeight)
	{
		try {
			if (isZoom) {
				return resizeZoomImage(ImageIO.read(new File(imgName)), type, maxWidth, maxHeight);
			} else {
	 ...
	 *            int
	 */
	public static BufferedImage resizeImage(BufferedImage image, int type, int maxWidth,
			int maxHeight)
	{

		return createHeadlessSmoothBufferedImage(image, type, maxWidth, maxHeight);
	}

	/**
	 * ZoomResizes an image.
	 * 
	 * @param image
	 *            The image to resize
	 * @param maxWidth
	 ...
	 *            int
	 */
	public static BufferedImage resizeZoomImage(BufferedImage image, int type, int maxWidth,
			int maxHeight)
	{
	 ...
	 *            The filename to use
	 * @param type
	 *            The image type. Use <code>ImageUtils.IMAGE_JPEG</code> to save as JPEG images, or
	...
	/**
	 * Creates a <code>BufferedImage</code> from an <code>Image</code>. This method can function on
	 * a completely headless system. This especially includes Linux and Unix systems that do not
	 * have the X11 libraries installed, which are required for the AWT subsystem to operate. This
	 * method uses nearest neighbor approximation, so it's quite fast. Unfortunately, the result is
	 * nowhere near as nice looking as the createHeadlessSmoothBufferedImage method.
	 * 
	 * @param image
	 ...
	 *            int
	 */
	public static BufferedImage createHeadlessBufferedImage(BufferedImage image, int type,
			int width, int height)
	{
		if (type == ImageSizer.IMAGE_PNG && hasAlpha(image)) {
	 ...
	 * Creates a <code>BufferedImage</code> from an <code>Image</code>. This method can function on
	 * a completely headless system. This especially includes Linux and Unix systems that do not
	 * have the X11 libraries installed, which are required for the AWT subsystem to operate. The
	...
	public static BufferedImage createHeadlessSmoothBufferedImage(BufferedImage source, int type,
			int width, int height)
	{
		if (type == ImageSizer.IMAGE_PNG && hasAlpha(source)) {
			type = BufferedImage.TYPE_INT_ARGB;
		} else {
			type = BufferedImage.TYPE_INT_RGB;
		}
	...
	{
		BufferedImage bi = ImageSizer.resizeImage("d:/test.jpg", IMAGE_JPEG, ZOOM, 200, 200);
