...
/**
 * 分页时用到的where条件Bean 调用范例: Page<User> page=new Page<User>(1,1);
 * page.addConditions(Condition.getInstance().like("name", "张",MatchWay.ANYWHERE).in("name", new
 * Object[]{"王"})); page.addOrderProperties("name", true); page = userService.loadPage(page);
 * 
 * @author 袁明敏
 * @since 1.0
 * @see com.wt.hea.common.model.Page
 * 
 */
...
public final class Condition implements Serializable
{
	...
	/**
	 * Apply a constraint expressed in SQL, with the given JDBC parameter. Any occurrences of
	 * <tt>{alias}</tt> will be replaced by the table alias.
	 * 
	 * @param sql
	 *            sql语句串
	 * @param value
	...
	/**
	 * 方法说明： Apply an "equal" constraint to the named property
	 * 
	 * @param propertyName
	 *            属性名称
	 * @param value
	...
	public Condition in(String propertyName, Collection values)
	{
		...
		if (values.size() < pageSize) {
			...
			for (int i = 1; i <= map.size(); i++) {
				c = Restrictions.or(Restrictions.in(propertyName, map.get(i)),
						Restrictions.in(propertyName, map.get(i)));
	 ...
	 *         expressions.add(Expression.and(lhs, rhs)); return this; }
	 */
	 ...
	 * @return Condition
	 * 
	 *         public static LogicalExpression or(Condition lhs, Condition rhs) { return null; }
	 */
	 ...
	 * @return Condition
	 * 
	 *         public static Condition not(Condition expression) { return null; }
	 */

	/**
	 * Apply a constraint expressed in SQL, with the given JDBC parameters. Any occurrences of
	 * <tt>{alias}</tt> will be replaced by the table alias.
	 * 
	 * @param sql
	 *            带参的sql语句
